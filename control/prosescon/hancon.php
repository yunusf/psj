...
if(empty($_GET['thisposition'])){
	...
	if($to=='logincon'){
		...
		$ru->addproduct($con,$_POST['id_produk'],$_POST['id_kat_produk'],$_POST['nama'],$har,$_POST['stok'],$gambar,$_POST['tanggal'],$_POST['deskripsi']);
	}
	else if($to=='editproduct'){
		$har= str_replace(".", "", $_POST['harga']);
		if(isset($_POST['tetap'])){
			$ru->editproductq($con,$_POST['id_produk'],$_POST['id_kat_produk'],$_POST['nama'],$har,$_POST['stok'],$_POST['tanggal'],$_POST['deskripsi']);
		}	
		else
		{
			...
			$loc="../img/produk/$name";$gambar="img/produk/$name";
			move_uploaded_file($tmp_name,$loc);
			$ru->editproduct($con,$_POST['id_produk'],$_POST['id_kat_produk'],$_POST['nama'],$har,$_POST['stok'],$gambar,$_POST['tanggal'],$_POST['deskripsi']);
		}
	}
	...
	else if($to=='addgallery'){
		$tmp_name=$_FILES['gambar']['tmp_name'];
		$name=$_FILES['gambar']['name'];
		$type=$_FILES['gambar']['type'];
		$loc="../img/gallery/$name";$gambar="img/gallery/$name";
		move_uploaded_file($tmp_name,$loc);
		$ru->addgallery($con,$gambar);
	}
	else if($to=='hapusgallery'){$sli=base64_decode($_GET['thissli']);
		$dd=mysqli_fetch_array(mysqli_query($con,"SELECT * from gallery where id_gallery='$sli'"));
			if($dd!=NULL)
			{
				unlink('../'.$dd['gambar']);
			}
		$ru->hapusgallery($con,$sli);
	}
