...
class con{
	...
	function addproduct($con,$id_produk,$id_kat_produk,$nama,$harga,$stok,$gambar,$tanggal,$des){ $tt=date('Y-m-d H:i:s');
		$sql=mysqli_query($con,"INSERT INTO produk set id_produk='$id_produk',id_kat_produk='$id_kat_produk',nama='$nama',harga='$harga',stok='$stok',gambar='$gambar',tanggal='$tt',deskripsi='$des'");
		if($sql){
	...
	function editproductq($con,$id_produk,$id_kat_produk,$nama,$harga,$stok,$tanggal,$des){  $tt=date('Y-m-d H:i:s');
		$sql=mysqli_query($con,"UPDATE produk set id_kat_produk='$id_kat_produk',nama='$nama',harga='$harga',stok='$stok',tanggal='$tt',deskripsi='$des' where id_produk='$id_produk' ");
		header('location:../main?thisaction='.base64_encode('listproduct').'');
	}
	function editproduct($con,$id_produk,$id_kat_produk,$nama,$harga,$stok,$gambar,$tanggal,$des){  $tt=date('Y-m-d H:i:s');
		$sql=mysqli_query($con,"UPDATE produk set id_kat_produk='$id_kat_produk',nama='$nama',harga='$harga',stok='$stok',gambar='$gambar',tanggal='$tt',deskripsi='$des' where id_produk='$id_produk' ");
		header('location:../main?thisaction='.base64_encode('listproduct').'');
	}
...
function addgallery($con,$gambar){
		$sql=mysqli_query($con,"INSERT INTO gallery set gambar='$gambar' ");
		header('location:../main?thisaction='.base64_encode('gallery').'');
	}
	function hapusgallery($con,$id_gallery){
		$sql=mysqli_query($con,"DELETE FROM gallery where id_gallery='$id_gallery' ");
		header('location:../main?thisaction='.base64_encode('gallery').'');
	}
